...
#toma el imput declarado mas abajo y lo transforma en dataframe como funcion recurrente
def Subida():
    
    df_input = pd.DataFrame()  
    df_total=pd.DataFrame()
    df_total=pd.read_excel(input)
    df_input=pd.read_excel(input)
    return df_input


#esta funcion renombra los campos determinados como fecha=ds y valores=y
def prep_data(Tabla):

    Tabla = Tabla.rename({date_col:"ds",metric_col:"y"},errors='raise',axis=1)
    st.markdown("La columna seleccionada se renombrara como: **ds** y los datos como : **y**")
    Tabla = Tabla[['ds','y']]
    Tabla =  Tabla.sort_values(by='ds',ascending=True)
    return Tabla
...
if page == "Aplicación":
    ...
    try:
        ...
        if input:
            with st.spinner('Loading data..'):
                ...
                columnis = list(df.columns)
                col3, = st.columns(1)
                with col3:
                    Prod_col = st.selectbox("Seleccionar la columna de Producto",index= 0,options=columnis,key="Productos")
               
                #Guardar en lugar fresco y seco, esta es la pieza clave para ajustar los productos    
                st.write("Seleccionar Producto a predecir:")
                a=list(df[Prod_col].unique())
                col4, = st.columns(1)
                Listado= st.selectbox("Producto",index= 0,options=a,key="elegir")
                Seleccion= df.loc[:, Prod_col] == Listado
                Tabla= df.loc[Seleccion]
                #st.write(Tabla)
    ...
    if st.button('Predecir',key='Aplica', ):
            ...
            #st.write(Salida)
            df_cv = cross_validation(basico, initial='90 days', period='2 days', horizon = '90 days')
        
            CV=df_cv.head(periods_input)
            fdg=CV[['ds','yhat','yhat_lower','yhat_upper','y']]
            fdg.to_excel("evaluacion.xlsx", index=0)
            fdg2=CV[['yhat','yhat_lower','yhat_upper','y']]
            dataframe=pd.DataFrame(CV)
            #pd.to_datetime(pd.Series(['05/23/2005']), format="%m/%d/%Y")
            CV['ds'] =  pd.to_datetime(df['ds'], format='%Y/%m/%d')
            pulento2=pd.DataFrame(data=fdg2)
            #pulento
            #df_p = performance_metrics(df_cv)
            #st.write(pulento)
            fdg['ds'].dt.strftime('%d/%m/%Y')
            
            chart_data =fdg
            st.write(chart_data)
            st.line_chart(pulento2)
